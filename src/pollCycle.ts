 ...
 * every registered recipient (src/recipientStore.ts — populated dynamically as the
 * bot is added to servers, see discordGateway.ts) when genuinely new sessions appear.
 * On the first-ever poll (state.hasPolledBefore === false) it only records a
 * baseline and never alerts, since there's nothing to compare against yet. On auth
 * failure it DMs a warning exactly once (tracked via state.authAlertSent) asking for
 * a fresh token, then stays silent on repeat failures until a poll succeeds again, at
 * which point the flag resets so a future failure alerts again. Returns the updated
 * PollState for the caller to persist; never throws — poll and Discord failures are
 * logged and swallowed so a single bad cycle doesn't crash the caller's loop.
 */
  ...
  try {
    ...
    if (state.hasPolledBefore && newEntries.length > 0) {
      ...
      if (recipients.length > 0) {
        ...
        log("warn", "New showtimes found but no recipients registered yet", {
          count: newEntries.length,
        });
      }
    }
